...
# Получаем текущую директорию и добавляем её в sys.path
now_dir = os.getcwd()
...
# Функция для печати строки с возможностью форматирования
...
# Функция фазового вокодера для обработки аудиосигналов
...
# Класс для обработки аудиосигналов с использованием multiprocessing
...
if __name__ == "__main__":
    ...
    # Устанавливаем устройство для PyTorch
    # device = torch.device(
    #     "cuda"
    ...
    # Класс GUI для управления интерфейсом
    class GUI:
        def __init__(self) -> None:
        ...
        def check_assets(self):
            ...
            if not check_all_assets(update=self.config.update):
                if self.config.update:
                    ...
                    if not check_all_assets(update=self.config.update):
                        printt("Не удалось удовлетворить все необходимые ресурсы.")
                        exit(1)

        def load(self):
            try:
                # Проверяем, существует ли файл конфигурации, если нет, копируем его из шаблона
                if not os.path.exists("configs/inuse/config.json"):
                    shutil.copy("configs/config.json", "configs/inuse/config.json")

                # Открываем файл конфигурации и загружаем данные
                with open("configs/inuse/config.json", "r") as j:
                    data = json.load(j)

                    # Устанавливаем флаги на основе типа модели и метода f0
                    ...
                    # Проверяем, существует ли указанный API хоста
                    if data["sg_hostapi"] in self.hostapis:
                        self.update_devices(hostapi_name=data["sg_hostapi"])

                        # Проверяем, существуют ли указанные устройства ввода и вывода
                        if (
                    ...
                    sg.Frame(
                        layout=[
                            [
                                ...
                                sg.Checkbox(
                                    i18n("Эксклюзивное использование WASAPI устройства"),
                            ...
                            [
                                sg.Text(i18n("Входное устройство")),
                                sg.Combo(
                                    self.input_devices,
                                    key="sg_input_device",
                            ...
                            [
                                sg.Text(i18n("Выходное устройство")),
                                sg.Combo(
                                    self.output_devices,
                            ...
                            [
                                sg.Button(i18n("Перезагрузить список устройств"), key="reload_devices"),
                                sg.Radio(
                                    i18n("Использовать частоту дискретизации модели"),
                                    "sr_type",
                                    key="sr_model",
                                ...
                                sg.Radio(
                                    i18n("Использовать частоту дискретизации устройства"),
                                    "sr_type",
                                    key="sr_device",
                                    default=data.get("sr_device", False),
                                    enable_events=True,
                                ),
                                sg.Text(i18n("Частота дискретизации:")),
                                sg.Text("", key="sr_stream"),
                            ],
                        ],
                        title=i18n("Аудио устройства"),
                    )
                ],
                [
                    sg.Frame(
                        layout=[
                            ...
                            [
                                sg.Text(i18n("Смещение форманта")),
                                sg.Slider(
                                    range=(-5, 5),
                                    key="formant",
                                    resolution=0.01,
                            ...
                            [
                                sg.Text(i18n("Алгоритм высоты тона")),
                                sg.Radio(
                                    "pm",
                                    "f0method",
                                    key="pm",
                        ...
                        title=i18n("Общие настройки"),
                    ),
                    sg.Frame(
                        layout=[
                            [
                                sg.Text(i18n("Длина блока")),
                                sg.Slider(
                                    range=(0.02, 1.5),
                                    key="block_time",
                                    resolution=0.01,
                            ...
                            # ],
                            [
                                sg.Text(i18n("Количество процессов harvest")),
                                sg.Slider(
                                    range=(1, n_cpu),
                                    key="n_cpu",
                                    resolution=1,
                            ...
                            [
                                sg.Text(i18n("Дополнительное время обработки")),
                                sg.Slider(
                                    range=(0.05, 5.00),
                                    key="extra_time",
                                    resolution=0.01,
                            ...
                            [
                                sg.Checkbox(
                                    i18n("Шумоподавление на входе"),
                                    key="I_noise_reduce",
                                    enable_events=True,
                                ),
                                sg.Checkbox(
                                    i18n("Шумоподавление на выходе"),
                                    key="O_noise_reduce",
                                    enable_events=True,
                                ),
                                sg.Checkbox(
                                    i18n("Использовать фазовый вокодер"),
                                    key="use_pv",
                            ...
                            # [sg.Text("Примечание: при первом использовании JIT ускорения может возникнуть задержка и шум, но это нормально!")],
                        ],
                        title=i18n("Настройки производительности"),
                    ),
                ],
                [
                    sg.Button(i18n("Начать преобразование аудио"), key="start_vc"),
                    sg.Button(i18n("Остановить преобразование аудио"), key="stop_vc"),
                    sg.Radio(
                        i18n("Мониторинг входа"),
                        "function",
                        key="im",
                        default=False,
                        enable_events=True,
                    ...
                    sg.Radio(
                        i18n("Изменение голоса на выходе"),
                        "function",
                        key="vc",
                        default=True,
                        enable_events=True,
                    ),
                    sg.Text(i18n("Задержка алгоритма (мс):")),
                    sg.Text("0", key="delay_time"),
                    sg.Text(i18n("Время обработки (мс):")),
        ...
        def event_handler(self):
            ...
            while True:
                ...
                if event == "threhold":
                    ...
                    # Другие параметры не поддерживают обновление в реальном времени
                    self.stop_stream()

        def set_values(self, values):
            # Проверяем, указан ли путь к файлу .pth
            if len(values["pth_path"].strip()) == 0:
                sg.popup(i18n("Пожалуйста, выберите файл .pth"))
                return False
            # Проверяем, указан ли путь к файлу .index
            if len(values["index_path"].strip()) == 0:
                sg.popup(i18n("Пожалуйста, выберите файл .index"))
                return False
            # Проверяем, содержат ли пути к файлам .pth и .index китайские символы
            pattern = re.compile("[^\x00-\x7F]+")
            if pattern.findall(values["pth_path"]):
                sg.popup(i18n("Путь к файлу .pth не должен содержать китайские символы"))
                return False
            if pattern.findall(values["index_path"]):
                sg.popup(i18n("Путь к файлу .index не должен содержать китайские символы"))
                return False
            # Устанавливаем устройства ввода и вывода
        ...
        def start_stream(self):
            ...
            if not flag_vc:
                flag_vc = True
                # Проверяем, используется ли эксклюзивный режим WASAPI
                if (
                    ...
                    extra_settings = None
                # Создаем и запускаем поток для обработки аудио
        ...
        def audio_callback(
            ...
            # Уменьшение шума на входе и ресемплинг
        ...
        # Смешивание огибающей громкости
        ...
        def set_devices(self, input_device, output_device):
            """Установка устройств ввода и вывода"""
